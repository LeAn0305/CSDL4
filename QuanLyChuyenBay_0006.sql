...
SELECT NV.MaNV , NV.Ten , COUNT(DISTINCT MB.Loai) [Tổng Số Loại Máy Bay]
FROM NHANVIEN NV
LEFT JOIN CHUNGNHAN CN ON NV.MaNV = CN.MaNV
LEFT JOIN MAYBAY MB ON CN.MaMB = MB.MaMB
GROUP BY NV.MaNV , NV.Ten

--Câu 2 : Với Mỗi Nhân Viên Cho Biết Mã Số , Tên Nhân Viên Và Tổng Số Loại Máy Bay Boeing Mà Nhân Viên Đó Có Thể Lái
SELECT NV.MaNV , NV.Ten , COUNT(DISTINCT MB.Loai) [Tổng Số Loại Máy Bay BOEING]
FROM NHANVIEN NV
LEFT JOIN CHUNGNHAN CN ON NV.MaNV = CN.MaNV
...
SELECT MB.Loai [Loại Máy Bay] , COUNT(DISTINCT CN.MaNV) [Tổng Số Phi Công Có Thể Lái]
FROM MAYBAY MB
...
SELECT MB.Loai [Loại Máy Bay] , COUNT(DISTINCT CB.MaCB) [Tổng Số Chuyến Bay Không Thể Thực Hiện]
FROM MAYBAY MB
...
SELECT DISTINCT MB.Loai [Loại Máy Bay] , COUNT(DISTINCT NV.MaNV) [Tổng Số Phi Công Có Lương Lớn Hơn 100,000]
FROM MAYBAY MB
LEFT JOIN CHUNGNHAN CN ON MB.MaMB = CN.MaMB
LEFT JOIN NHANVIEN NV ON CN.MaNV = NV.MaNV AND NV.Luong > 100000
GROUP BY MB.Loai 
...
SELECT MB.Loai [Loại Máy Bay] , AVG(NV.Luong) [Lương Trung Bình]
FROM MAYBAY MB
LEFT JOIN CHUNGNHAN CN ON MB.MaMB = CN.MaMB
...
SELECT MB.Loai [Loại Máy Bay] , COUNT(DISTINCT NV.MaNV) [Tổng Số Nhân Viên  Không Thể Lái]
FROM MAYBAY MB
...
SELECT MB.Loai [Loại Máy Bay] , COUNT(DISTINCT NV.MaNV) [Tổng Số Phi Công Không Thể Lái]
FROM MAYBAY MB
CROSS JOIN (
	SELECT DISTINCT NV.MaNV, NV.Ten
	FROM NHANVIEN NV
...
SELECT NV.MaNV , NV.Ten , COUNT(DISTINCT CB.MaCB) [Số Chuyến Bay Mà Nhân Viên Có Thể Lái Khi Xuất Phát Từ Sài Gòn]
FROM NHANVIEN NV
...
SELECT NV.MaNV , NV.Ten , COUNT(DISTINCT CB.MaCB) AS [Tổng Số Chuyến Bay Mà Phi Công Đó Có Thể Lái Từ Sài Gòn]
FROM (
	SELECT DISTINCT NV.MaNV, NV.Ten
	FROM NHANVIEN NV
	JOIN CHUNGNHAN CN ON NV.MaNV = CN.MaNV
...
SELECT CB.MaCB , COUNT(DISTINCT MB.Loai) [Tổng Số Loại Máy Bay Không Thể Thực Hiện]
FROM CHUYENBAY CB
CROSS JOIN MAYBAY MB
...
SELECT CB.MaCB , COUNT(DISTINCT MB.Loai) [Tổng Số Loại Máy Bay Có Thể Thực Hiện]
FROM CHUYENBAY CB
CROSS JOIN MAYBAY MB
WHERE MB.TamBay >= CB.DoDai
GROUP BY CB.MaCB
...
SELECT CB.MaCB , COUNT(DISTINCT NV.MaNV) [Tổng Số Nhân Viên Không Thể Lái]
FROM CHUYENBAY CB
CROSS JOIN NHANVIEN NV
WHERE NOT EXISTS(
					SELECT 1
...
SELECT CB.MaCB, COUNT(DISTINCT NV.MaNV) AS [Tổng Số Phi Công Không Thể Lái]
FROM CHUYENBAY CB
CROSS JOIN (
	SELECT DISTINCT NV.MaNV, NV.Ten
	FROM NHANVIEN NV
	JOIN CHUNGNHAN CN ON NV.MaNV = CN.MaNV
) NV -- lọc ra chỉ những phi công
WHERE NOT EXISTS (
	SELECT 1
	FROM CHUNGNHAN CN
	JOIN MAYBAY MB ON CN.MaMB = MB.MaMB
